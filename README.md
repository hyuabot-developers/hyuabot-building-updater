...
A one-time data loader that scrapes Hanyang University's campus portal and populates the HYUabot database with building locations and classroom information.

## Overview

This job runs once at initial deployment. It scrapes the university portal using BeautifulSoup4 and inserts records into the `building` and `room` tables.

## Architecture

```
src/
├── main.py           # Entry point; orchestrates building and room loading
├── models.py         # SQLAlchemy ORM models (Building, Room)
└── utils/
...
```bash
docker build -t hyuabot-building-updater .
